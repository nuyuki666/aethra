...
struct inputbox_anim {
    float cursor_alpha, cursor_pos_x;
    float text_alpha = 0.17f;
};
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    static std::map <ImGuiID, inputbox_anim> anim;
    ...
    if (render_cursor || render_selection)
    {
        ...
        {
            it_anim->second.text_alpha = ImLerp(it_anim->second.text_alpha,
                (render_cursor && !is_displaying_hint) ? 0.9f : (render_cursor && is_displaying_hint) ? 0.f : 0.17f,
                ImGui::GetIO().DeltaTime * 14.f);
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos - draw_scroll, ImColor(1.f, 1.f, 1.f, it_anim->second.text_alpha * style.Alpha), buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
        ...
        if (render_cursor)
        {
            ...
            if (it_anim->second.cursor_pos_x == 0) it_anim->second.cursor_pos_x = cursor_screen_pos.x;

            it_anim->second.cursor_pos_x = ImLerp(it_anim->second.cursor_pos_x, cursor_screen_pos.x, ImGui::GetIO().DeltaTime * 14.f);
            ImRect cursor_screen_rect(it_anim->second.cursor_pos_x, cursor_screen_pos.y - g.FontSize + 0.5f, it_anim->second.cursor_pos_x + 1.0f, cursor_screen_pos.y - 1.5f);
    ...
    else
    {
        ...
        if (is_multiline || (buf_display_end - buf_display) < buf_display_max_length)
        {
            it_anim->second.text_alpha = ImLerp(it_anim->second.text_alpha, is_displaying_hint ? 0.17f : 0.9f, ImGui::GetIO().DeltaTime * 14.f);
            draw_window->DrawList->AddText(g.Font, g.FontSize, draw_pos, ImColor(1.f,1.f,1.f, it_anim->second.text_alpha * style.Alpha), buf_display, buf_display_end, 0.0f, is_multiline ? NULL : &clip_rect);
        }
    }
